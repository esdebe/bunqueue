---
title: High-Performance Job Queue for Bun
description: bunqueue is the fastest job queue for Bun with SQLite persistence, dead letter queues, cron scheduling, priority jobs, and S3 backups. No Redis required.
template: splash
head:
  - tag: meta
    attrs:
      property: 'og:image'
      content: 'https://bunqueue.dev/og-image.png'
hero:
  tagline: The fastest job queue for Bun. Zero Redis. Zero infrastructure dependencies. Production ready.
  image:
    file: ../../assets/hero.svg
  actions:
    - text: Get Started
      link: /guide/introduction/
      icon: right-arrow
      variant: primary
    - text: GitHub
      link: https://github.com/egeominotti/bunqueue
      icon: github
      variant: minimal
---

import { Card, CardGrid, Tabs, TabItem, Badge, LinkCard } from '@astrojs/starlight/components';

## Quick Start

<Tabs>
  <TabItem label="Embedded Mode">
    ```typescript
    import { Queue, Worker } from 'bunqueue/client';

    // Create a queue
    const queue = new Queue('emails', { embedded: true });

    // Add jobs
    await queue.add('welcome', {
      to: 'user@example.com',
      template: 'welcome'
    });

    // Process jobs
    const worker = new Worker('emails', async (job) => {
      await sendEmail(job.data);
      return { sent: true };
    }, { embedded: true });

    worker.on('completed', (job) => console.log(`✓ ${job.id}`));
    ```
  </TabItem>
  <TabItem label="Server Mode">
    ```bash
    # Start the server
    bunqueue start --data-path ./data/queue.db

    # Or with Docker
    docker run -d -p 6789:6789 -p 6790:6790 \
      -v bunqueue-data:/app/data \
      ghcr.io/egeominotti/bunqueue:latest
    ```

    ```typescript
    // Then, in your application
    import { Queue, Worker } from 'bunqueue/client';

    // Create a queue (connects to localhost:6789)
    const queue = new Queue('emails');

    // Add jobs
    await queue.add('welcome', {
      to: 'user@example.com',
      template: 'welcome'
    });

    // Process jobs
    const worker = new Worker('emails', async (job) => {
      await sendEmail(job.data);
      return { sent: true };
    });

    worker.on('completed', (job) => console.log(`✓ ${job.id}`));
    ```
  </TabItem>
</Tabs>

---

## Why bunqueue?

<CardGrid>
  <Card title="Blazing Fast" icon="rocket">
    Built on Bun's native SQLite with WAL mode. Optimized data structures deliver sub-millisecond latency at scale.
  </Card>
  <Card title="Zero Infrastructure Dependencies" icon="approve-check-circle">
    No Redis, no external services. Just Bun and SQLite. Deploy anywhere in seconds.
  </Card>
  <Card title="Production Ready" icon="approve-check">
    Stall detection, dead letter queues, automatic retries, rate limiting, and S3 backups built-in.
  </Card>
  <Card title="BullMQ Compatible" icon="puzzle">
    Familiar API for easy migration. Drop-in replacement for most BullMQ use cases.
  </Card>
</CardGrid>

---

## Deployment Modes

bunqueue offers two deployment modes to fit your architecture. Choose based on your use case.

<CardGrid>
  <Card title="Embedded Mode" icon="seti:db">
    Run bunqueue directly inside your application process. Zero network overhead, instant setup.

    **Best for:**
    - Single-process applications
    - Serverless / Edge functions
    - Prototyping and development
    - Simple background job processing
  </Card>
  <Card title="TCP Server Mode" icon="server">
    Run bunqueue as a standalone server. Multiple clients connect via TCP protocol.

    **Best for:**
    - Microservices architecture
    - Multiple worker processes
    - Language-agnostic clients (HTTP/TCP API)
    - Centralized job management
  </Card>
</CardGrid>

### Mode Comparison

| Aspect | Embedded | TCP Server |
|--------|----------|------------|
| **Setup** | Zero config | Run `bunqueue start` |
| **Network** | None (in-process) | TCP :6789 / HTTP :6790 |
| **Option** | `embedded: true` | Default (no option) |
| **Persistence** | `DATA_PATH` env var | `--data-path` flag |
| **Scaling** | Single process | Multiple clients |
| **Latency** | Lowest (~μs) | Low (~ms) |

### Embedded Mode

```typescript
import { Queue, Worker } from 'bunqueue/client';

// Both Queue and Worker must have embedded: true
const queue = new Queue('tasks', { embedded: true });

const worker = new Worker('tasks', async (job) => {
  return await processJob(job.data);
}, { embedded: true });
```

### TCP Server Mode

```bash
# Terminal 1: Start the server
bunqueue start --data-path ./data/queue.db
```

```typescript
// Terminal 2: Your application connects to the server
import { Queue, Worker } from 'bunqueue/client';

// No embedded option = connects to localhost:6789
const queue = new Queue('tasks');
const worker = new Worker('tasks', async (job) => {
  return await processJob(job.data);
});
```

:::caution[Mode Mismatch]
Queue and Worker must use the **same mode**. Mixing embedded Queue with TCP Worker (or vice versa) causes connection timeouts.
:::

<div style="text-align: center; margin-top: 1rem;">
  <a href="/guide/introduction/" style="color: var(--sl-color-text-accent);">
    Learn more about deployment modes →
  </a>
</div>

---

## Performance

Measured on Mac Studio M1 Max, 32GB RAM. Run `bun run bench` for your hardware.

<CardGrid>
  <Card title="50-150K ops/sec" icon="star">
    Push throughput with batch operations
  </Card>
  <Card title="&lt; 1ms p99" icon="stopwatch">
    Sub-millisecond latency
  </Card>
  <Card title="~50MB base" icon="laptop">
    Minimal memory footprint
  </Card>
  <Card title="&lt; 100ms" icon="seti:clock">
    Cold start time
  </Card>
</CardGrid>

<div style="text-align: center; margin-top: 1rem;">
  <a href="/guide/benchmarks/" style="color: var(--sl-color-text-accent);">
    Run benchmarks yourself →
  </a>
</div>

---

## Features

<CardGrid>
  <Card title="Priority Queues" icon="bars">
    Process high-priority jobs first with configurable priority levels.
  </Card>
  <Card title="Delayed Jobs" icon="stopwatch">
    Schedule jobs to run at a specific time or after a delay.
  </Card>
  <Card title="Retries & Backoff" icon="approve-check-circle">
    Automatic retries with exponential backoff on failure.
  </Card>
  <Card title="Rate Limiting" icon="setting">
    Control job processing rate per queue or globally.
  </Card>
  <Card title="Stall Detection" icon="warning">
    Automatic recovery of jobs that become unresponsive.
  </Card>
  <Card title="Dead Letter Queue" icon="error">
    Advanced DLQ with metadata, filtering, and auto-retry.
  </Card>
  <Card title="Cron Jobs" icon="seti:clock">
    Schedule recurring jobs with cron expressions or intervals.
  </Card>
  <Card title="Parent-Child Flows" icon="document">
    Create job hierarchies with dependencies.
  </Card>
  <Card title="Progress Tracking" icon="up-caret">
    Real-time progress updates and job logs.
  </Card>
  <Card title="Webhooks" icon="external">
    HTTP callbacks for job events.
  </Card>
  <Card title="S3 Backup" icon="cloud">
    Automated backups to any S3-compatible storage.
  </Card>
  <Card title="Prometheus Metrics" icon="laptop">
    Built-in metrics endpoint for monitoring.
  </Card>
</CardGrid>

---

## Enterprise Ready

<CardGrid>
  <LinkCard
    title="High Availability"
    description="SQLite WAL mode with S3 backup for disaster recovery."
    href="/guide/backup/"
  />
  <LinkCard
    title="Observability"
    description="Prometheus metrics, structured logs, and webhooks."
    href="/guide/webhooks/"
  />
  <LinkCard
    title="Security"
    description="Token-based authentication and rate limiting."
    href="/guide/rate-limiting/"
  />
  <LinkCard
    title="TypeScript"
    description="Full type safety with comprehensive type definitions."
    href="/api/http/"
  />
</CardGrid>
