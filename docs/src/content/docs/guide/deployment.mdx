---
title: "Bunqueue Production Deployment Guide: Docker, Systemd & PM2"
description: Deploy bunqueue to production with Docker, systemd, or PM2. S3 backups, health checks, resource limits, and disaster recovery strategies.
head:
  - tag: meta
    attrs:
      property: og:image
...
<Aside type="caution">
Set `DATA_PATH` environment variable or use a [configuration file](/guide/configuration/) for SQLite persistence. Without it, data is stored **in-memory only** and will be lost on restart.
</Aside>
...
<Aside type="tip" title="Config File">
Instead of environment variables, you can centralize all settings in a typed `bunqueue.config.ts`:
...
|----------|-------------|---------|
| `DATA_PATH` | SQLite database path | `in-memory` |
