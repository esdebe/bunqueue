---
title: MCP Server for AI Agents
description: Use bunqueue MCP server with Claude Desktop, Claude Code, Cursor, and any MCP client. 66 tools for complete queue management via natural language.
head:
  - tag: meta
    attrs:
      property: og:image
      content: https://bunqueue.dev/og/integrations.png
---

import { Tabs, TabItem, Aside, Badge } from '@astrojs/starlight/components';

bunqueue includes an **MCP (Model Context Protocol)** server that gives AI assistants **complete control** over your job queues. 66 tools, 5 resources, zero configuration. Built on the official `@modelcontextprotocol/sdk` for full protocol compliance.

## What is MCP?

MCP is an open protocol that enables AI assistants to interact with external tools and services. With bunqueue's MCP server, you can:

- Add, pull, ack, and fail jobs using natural language
- Monitor queue health, memory, and Prometheus metrics
- Configure cron jobs, rate limits, and concurrency
- Debug failed jobs in the DLQ
- Manage workers, webhooks, and locks
- Full pull/ack/fail cycle for job consumption

<Aside type="tip">
MCP works with Claude Desktop, Claude Code (CLI), Cursor, Windsurf, and any MCP-compatible client.
</Aside>

## Installation

The MCP server is included with bunqueue. No additional installation required.

```bash
bun add bunqueue
```

## Configuration

### Claude Desktop

<Tabs>
<TabItem label="macOS">
```json
// ~/Library/Application Support/Claude/claude_desktop_config.json
{
  "mcpServers": {
    "bunqueue": {
      "command": "bunx",
      "args": ["bunqueue-mcp"]
    }
  }
}
```
</TabItem>
<TabItem label="Windows">
```json
// %APPDATA%\Claude\claude_desktop_config.json
{
  "mcpServers": {
    "bunqueue": {
      "command": "bunx",
      "args": ["bunqueue-mcp"]
    }
  }
}
```
</TabItem>
<TabItem label="Linux">
```json
// ~/.config/Claude/claude_desktop_config.json
{
  "mcpServers": {
    "bunqueue": {
      "command": "bunx",
      "args": ["bunqueue-mcp"]
    }
  }
}
```
</TabItem>
</Tabs>

### Claude Code (CLI)

```bash
claude mcp add bunqueue -- bunx bunqueue-mcp
```

### Cursor / Windsurf

Add to your MCP settings:

```json
{
  "mcpServers": {
    "bunqueue": {
      "command": "bunx",
      "args": ["bunqueue-mcp"]
    }
  }
}
```

### Remote Server (TCP Mode)

Connect to a running bunqueue server instead of embedded SQLite:

```json
{
  "mcpServers": {
    "bunqueue": {
      "command": "bunx",
      "args": ["bunqueue-mcp"],
      "env": {
        "BUNQUEUE_MODE": "tcp",
        "BUNQUEUE_HOST": "localhost",
        "BUNQUEUE_PORT": "6789",
        "BUNQUEUE_TOKEN": "your-auth-token"
      }
    }
  }
}
```

## Connection Modes

|                  | Embedded (default)           | TCP (remote)                     |
| ---------------- | ---------------------------- | -------------------------------- |
| **How it works** | Direct SQLite access         | Connects to bunqueue server      |
| **Setup**        | Zero config                  | Set `BUNQUEUE_MODE=tcp`          |
| **Best for**     | Local dev, single-machine    | Production, shared queues        |
| **Data**         | `./data/bunq.db`             | Remote server handles storage    |

## Available Tools <Badge text="66 tools" variant="success" />

### Job Operations (11 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_add_job` | Add a job to a queue |
| `bunqueue_add_jobs_bulk` | Add multiple jobs in one call |
| `bunqueue_get_job` | Get job details by ID |
| `bunqueue_get_job_state` | Get current state (waiting/delayed/active/completed/failed) |
| `bunqueue_get_job_result` | Get the result of a completed job |
| `bunqueue_cancel_job` | Cancel a waiting or delayed job |
| `bunqueue_promote_job` | Promote delayed job to waiting |
| `bunqueue_update_progress` | Update job progress (0-100) |
| `bunqueue_get_children_values` | Get child job results (FlowProducer) |
| `bunqueue_get_job_by_custom_id` | Look up job by custom ID |
| `bunqueue_wait_for_job` | Wait for a job to complete |

### Job Management (6 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_update_job_data` | Update job payload data |
| `bunqueue_change_job_priority` | Change job priority |
| `bunqueue_move_to_delayed` | Move job to delayed state |
| `bunqueue_discard_job` | Permanently discard a job |
| `bunqueue_get_progress` | Get progress value and message |
| `bunqueue_change_delay` | Change delay of a delayed job |

### Job Consumption (8 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_pull_job` | Pull a job from a queue for processing |
| `bunqueue_pull_job_batch` | Pull multiple jobs at once |
| `bunqueue_ack_job` | Acknowledge job completion with result |
| `bunqueue_ack_job_batch` | Batch acknowledge multiple jobs |
| `bunqueue_fail_job` | Mark a job as failed |
| `bunqueue_job_heartbeat` | Send heartbeat for active job |
| `bunqueue_job_heartbeat_batch` | Batch heartbeat for multiple jobs |
| `bunqueue_extend_lock` | Extend lock on an active job |

### Queue Control (11 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_list_queues` | List all queues |
| `bunqueue_count_jobs` | Count total jobs in a queue |
| `bunqueue_get_jobs` | List jobs with state filter and pagination |
| `bunqueue_get_job_counts` | Job counts per state |
| `bunqueue_pause_queue` | Pause job processing |
| `bunqueue_resume_queue` | Resume processing |
| `bunqueue_drain_queue` | Remove all waiting jobs |
| `bunqueue_obliterate_queue` | Remove ALL data from a queue |
| `bunqueue_clean_queue` | Remove old jobs by grace period |
| `bunqueue_is_paused` | Check if queue is paused |
| `bunqueue_get_counts_per_priority` | Job counts by priority level |

### Dead Letter Queue (4 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_get_dlq` | Get failed jobs from DLQ |
| `bunqueue_retry_dlq` | Retry jobs from DLQ |
| `bunqueue_purge_dlq` | Clear all DLQ entries |
| `bunqueue_retry_completed` | Reprocess completed jobs |

### Cron Jobs (4 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_add_cron` | Add recurring job (cron pattern or interval) |
| `bunqueue_list_crons` | List all scheduled crons |
| `bunqueue_get_cron` | Get cron details by name |
| `bunqueue_delete_cron` | Delete a cron job |

### Rate Limiting & Concurrency (4 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_set_rate_limit` | Set max jobs per second |
| `bunqueue_clear_rate_limit` | Remove rate limit |
| `bunqueue_set_concurrency` | Set max concurrent jobs |
| `bunqueue_clear_concurrency` | Remove concurrency limit |

### Webhook Management (4 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_add_webhook` | Add webhook for job events |
| `bunqueue_remove_webhook` | Remove a webhook |
| `bunqueue_list_webhooks` | List all webhooks |
| `bunqueue_set_webhook_enabled` | Enable/disable a webhook |

### Worker Management (3 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_register_worker` | Register a new worker |
| `bunqueue_unregister_worker` | Remove a worker |
| `bunqueue_worker_heartbeat` | Send worker heartbeat |

### Monitoring & Stats (11 tools)

| Tool | Description |
|------|-------------|
| `bunqueue_get_stats` | Global server statistics |
| `bunqueue_get_queue_stats` | Per-queue statistics |
| `bunqueue_list_workers` | List active workers |
| `bunqueue_get_job_logs` | Get job log entries |
| `bunqueue_add_job_log` | Add log entry to a job |
| `bunqueue_get_storage_status` | Disk health status |
| `bunqueue_get_per_queue_stats` | Detailed per-queue breakdown |
| `bunqueue_get_memory_stats` | Memory usage stats |
| `bunqueue_get_prometheus_metrics` | Prometheus exposition format |
| `bunqueue_clear_job_logs` | Clear logs for a job |
| `bunqueue_compact_memory` | Force memory compaction |

## MCP Resources <Badge text="5 resources" variant="note" />

Resources provide read-only context that AI assistants can access:

| Resource URI | Description |
|-------------|-------------|
| `bunqueue://stats` | Global server statistics |
| `bunqueue://queues` | All queues with job counts |
| `bunqueue://crons` | Scheduled cron jobs |
| `bunqueue://workers` | Active workers |
| `bunqueue://webhooks` | Registered webhooks |

## Error Handling

All 66 tools return structured error responses. When a tool encounters an error (backend exception, connection failure, invalid operation), it returns:

```json
{
  "error": "Human-readable error message"
}
```

With the MCP `isError: true` flag set, so AI clients can detect failures programmatically.

**Not-found responses** for `bunqueue_get_job`, `bunqueue_get_job_by_custom_id`, `bunqueue_get_progress`, and `bunqueue_get_cron` also return `isError: true` when the requested resource doesn't exist.

No raw stack traces or internal details are ever exposed to AI clients.

## MCP Prompts

bunqueue includes 3 built-in MCP prompts — pre-built templates that guide AI agents through complex diagnostic workflows.

### `bunqueue_health_report`

Generates a comprehensive health report with severity levels (OK/WARNING/CRITICAL). Gathers system stats, storage status, per-queue job counts, worker health, cron jobs, and memory usage in a single call.

### `bunqueue_debug_queue`

Deep diagnostic of a specific queue. Takes a `queue` argument and gathers job state breakdown, pause status, DLQ entries, active jobs, and priority distribution.

### `bunqueue_incident_response`

Incident response playbook for "jobs not processing". Takes an optional `queue` argument. Identifies backed-up queues, checks worker health, flags problematic queues (paused, stuck, or with DLQ entries), and provides a step-by-step triage checklist.

---

## Example Prompts

### Adding Jobs

```
"Add a job to the emails queue to send a welcome email to user@example.com"
```

```
"Add 100 jobs to the image-processing queue with URLs from this list..."
```

### Queue Management

```
"Show me all queues and how many jobs are waiting"
```

```
"Pause the payments queue - we have an issue"
```

```
"Drain all waiting jobs from the notifications queue"
```

### Monitoring & Debugging

```
"What are the current queue stats?"
```

```
"Show me failed jobs in the orders queue"
```

```
"Retry all jobs in the DLQ for the emails queue"
```

```
"Show me memory usage and Prometheus metrics"
```

### Cron Jobs

```
"Create a cron job that runs every hour to clean up old sessions"
```

```
"List all scheduled cron jobs"
```

### Worker Management

```
"List all active workers and their processing stats"
```

```
"Set rate limit to 50 jobs/sec on the api-calls queue"
```

### Job Consumption

```
"Pull 5 jobs from the tasks queue and show me their data"
```

```
"Mark job abc-123 as completed with result {processed: true}"
```

## Complete Example

Here's a full workflow using Claude with bunqueue MCP:

**You:** "I need to set up an email queue system"

**Claude:** *Creates queue, adds test job via `bunqueue_add_job`*

```json
{
  "jobId": "019c0e14-19ed-7000-9c72-d81ae9d00938"
}
```

**You:** "Add a cron job to send daily digest emails at 9 AM"

**Claude:** *Uses `bunqueue_add_cron`*

```json
{
  "name": "daily-digest",
  "queue": "emails",
  "schedule": "0 9 * * *",
  "nextRun": "2026-02-24T09:00:00.000Z"
}
```

**You:** "Set rate limit to 10 emails per second"

**Claude:** *Uses `bunqueue_set_rate_limit`*

**You:** "What's the current status?"

**Claude:** *Uses `bunqueue_get_stats` + `bunqueue_get_per_queue_stats`*

```json
{
  "waiting": 5,
  "active": 2,
  "completed": 143,
  "dlq": 1,
  "cronJobs": 1,
  "uptime": 3600
}
```

## Data Persistence

The MCP server uses SQLite for persistence. Default location:

```
./data/bunq.db
```

Configure with environment variable:

```json
{
  "mcpServers": {
    "bunqueue": {
      "command": "bunx",
      "args": ["bunqueue-mcp"],
      "env": {
        "DATA_PATH": "/var/lib/queue.db"
      }
    }
  }
}
```

## Processing Jobs

<Aside type="caution">
The MCP server **manages** queues. You need a separate Worker to **process** jobs.
</Aside>

Run a worker alongside the MCP server:

```typescript
// worker.ts
import { Worker } from 'bunqueue/client';

new Worker('emails', async (job) => {
  const { to, subject, body } = job.data;

  await job.updateProgress(50, 'Sending email...');
  await sendEmail(to, subject, body);
  await job.log('Email sent successfully');

  return { sent: true, timestamp: Date.now() };
}, { embedded: true });

console.log('Worker started, processing emails queue...');
```

```bash
bun run worker.ts
```

## Environment Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `BUNQUEUE_MODE` | `embedded` | Connection mode: `embedded` or `tcp` |
| `BUNQUEUE_HOST` | `localhost` | TCP server host |
| `BUNQUEUE_PORT` | `6789` | TCP server port |
| `BUNQUEUE_TOKEN` | - | Authentication token for TCP |
| `DATA_PATH` | `./data/bunq.db` | SQLite database path (embedded mode) |

## Troubleshooting

### MCP server not connecting

```bash
# Verify installation
bunx bunqueue-mcp --help

# Check if Bun is in PATH
which bun

# Test manually
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"1.0"}}}' | bunx bunqueue-mcp
```

### Jobs stuck in waiting

Make sure a Worker is running:

```bash
bun run worker.ts
```

### Data not persisting

Check file permissions:

```bash
ls -la ./data/
# Should show bunq.db with write permissions
```

### Claude not seeing tools

1. Restart Claude Desktop/Code
2. Check config file syntax (valid JSON)
3. Verify path to `bunqueue-mcp` is correct
4. Run `bunx bunqueue-mcp` manually to check for errors
