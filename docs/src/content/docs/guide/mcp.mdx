...
description: Use bunqueue MCP server with Claude Desktop, Claude Code, Cursor, and any MCP client. 70 tools, 3 prompts, 5 resources for complete queue management.
head:
  - tag: meta
    attrs:
      property: og:image
...
<CardGrid>
  <Card title="70 Tools" icon="puzzle">
    Job lifecycle, queue control, cron, DLQ, rate limiting, webhooks, monitoring, workflows
  </Card>
...
┌──────────────────────────────┐
│     bunqueue MCP Server      │
│  70 tools · 3 prompts · 5 resources  │
└──────────┬───────────────────┘
           │
     ┌─────┴─────┐
     ▼           ▼
...
## Available Tools <Badge text="70 tools" variant="success" />
...
All 70 tools return structured error responses with the MCP `isError: true` flag:
