...
head:
  ...
  - tag: meta
    attrs:
      name: keywords
      content: MCP, Model Context Protocol, AI agent, job queue, task scheduler, Claude, Cursor, Windsurf, bunqueue, cron jobs, HTTP handlers, dead letter queue, rate limiting
---
...
```
Agent registers handler → MCP spawns Worker → Worker pulls jobs →
Worker calls HTTP endpoint → Response saved as job result
```
