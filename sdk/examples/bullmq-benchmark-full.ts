/**
 * BullMQ Full Benchmark - Memory, Latency, Throughput
 * Direct comparison with flashQ
 */
...
console.log('📦 BullMQ Full Benchmark (Memory + Latency + Throughput)');
console.log('='.repeat(70));
...
const queue = new Queue('bullmq-benchmark-full', { connection });

// Get initial memory
const initialStats = await getDockerStats('redis-benchmark');
console.log(`\n📊 Initial Redis memory: ${initialStats?.memoryMB.toFixed(1) || 'N/A'} MB`);
...
// Memory after push
const afterPushStats = await getDockerStats('redis-benchmark');
console.log(`\n✅ Push complete: ${pushRate.toLocaleString()} jobs/sec (${(pushTime / 1000).toFixed(2)}s)`);
console.log(`📊 Redis memory after push: ${afterPushStats?.memoryMB.toFixed(1) || 'N/A'} MB`);
...
const progressInterval = setInterval(async () => {
  ...
  const stats = await getDockerStats('redis-benchmark');
...
// Final memory
const finalStats = await getDockerStats('redis-benchmark');
...
console.log('-'.repeat(70));
console.log('MEMORY (Redis server)');
