/**
 * Realistic Benchmark - 100K jobs with simulated CPU work
 */
...
// Simulate some CPU work (hash computation)
function doWork(iterations: number): number {
  let hash = 0;
...
console.log(`Work per job: 100 hash iterations`);
console.log('='.repeat(60));
...
for (let w = 0; w < NUM_WORKERS; w++) {
  const worker = new Worker('realistic-benchmark', async (job) => {
    // Do actual CPU work
    const result = doWork((job.data as any).workload || 100);
    return { hash: result };
  }, {
...
const progressInterval = setInterval(() => {
  ...
  const eta = processed > 0 ? Math.round((TOTAL_JOBS - processed) / avgRate) : 0;

  console.log(`   ${processed.toLocaleString()}/${TOTAL_JOBS.toLocaleString()} (${pct}%) | Current: ${currentRate.toLocaleString()}/s | Avg: ${avgRate.toLocaleString()}/s | ETA: ${eta}s`);
