/**
 * BullMQ CPU-Bound Benchmark - Direct comparison with flashQ
 * Same configuration and workload
 */
...
// Same CPU work as flashQ benchmark
...
console.log('📦 BullMQ CPU-Bound Benchmark (for comparison)');
console.log('='.repeat(70));
...
const queue = new Queue('bullmq-cpu-benchmark', { connection });
...
for (let w = 0; w < NUM_WORKERS; w++) {
  const worker = new Worker(
    'bullmq-cpu-benchmark',
    async (job: Job) => {
      const result = cpuIntensiveWork(job.data);
      return { index: job.data.index, ...result };
    },
    {
      connection,
  ...
  worker.on('completed', (job: Job, result: any) => {
    ...
    if (job.data.index !== result.index) {
      dataErrors++;
    }
  });
...
// Results
console.log('\n' + '='.repeat(70));
console.log('📊 BullMQ CPU-Bound RESULTS');
console.log('='.repeat(70));
